...
if(False):
   print( '<head><title>ERROR</title></head><body style="margin: 0px; height: 100%; width: 100%;" tabindex="0" class="jspsych-display-element">ERROR: {}</body></html>'.format(result) )
    ...
    json_file = open("/home/websites/yulabexp-face/22AY01/Demo_data/Demo" + input_data["userid"].value + ".txt",'w')
    json_file.write(input_data["Responses"].value)
    json_file.close()
    csv_file = open("/home/websites/yulabexp-face/22AY01/Demo_data/Demo" + input_data["userid"].value + ".csv",'w')
